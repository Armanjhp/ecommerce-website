        ...
        <div class="history">
            <?php
                try {
                    ...
                    $stmt = $conn->prepare("SELECT w.id, w.productid, w.dateadded, p.product_name, p.product_image FROM wishlist w" 
                        . " INNER JOIN products p ON w.productid = p.id WHERE w.uid = ".$_SESSION['id']." ORDER BY w.dateadded DESC");
                    $stmt->execute();
                    $ctr = 1;
                    ...
                    foreach ($stmt->fetchAll() as $display) {
                        echo '<div class="card">';
                            echo '<div class="item'.$ctr.'">';
                                echo '<table>';
                                    echo '<tr>';
                                        echo '<td colspan="2">Date: '.$display["dateadded"].'</td>';
                                    echo '</tr>';
                                    echo '<tr>';
